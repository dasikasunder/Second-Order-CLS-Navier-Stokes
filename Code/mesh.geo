lc = 0.02; // Define mesh characteristic length
...
Field[1].VIn = lc/4.;
Field[1].VOut = lc;
Field[1].XMax = xc+5.0*R;
Field[1].XMin = xc-3.0*R;
Field[1].YMax = yc+3.0*R;
Field[1].YMin = yc-3.0*R;
Background Field = 1;
*/

Physical Curve("9") = {8};       // Inlet
Physical Curve("230") = {1,2,3,4};  // Walls (Cylinder)
Physical Curve("231") = {5,7};  // Walls (Channel)
